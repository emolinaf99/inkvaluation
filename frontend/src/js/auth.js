...
export const authService = {
  ...
  async checkAuth() {
    try {
      const { data, error } = await useApi('/api/auth/check', 'GET');
  ...
  async logout() {
    try {
      const { data, error } = await useApi('/api/auth/logout', 'POST');
  ...
  async refreshToken() {
    try {
      const { data, error } = await useApi('/api/auth/refresh-token', 'POST');
