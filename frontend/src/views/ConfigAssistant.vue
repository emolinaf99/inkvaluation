...
<template>
    ...
    <section v-else class="sectionGeneralConfAssistant ux-container">
        <h1 class="ux-header">{{ $t('Configuración de asistente personal') }}</h1>

        <div class="contenedorSecConfAssistant ux-content">
            <div class="linkAndPreview ux-card ux-stagger-1">
                <button class="btnPreview BGBlue ux-button ux-hover-lift">{{ $t('Previsualiza') }}</button>
                <div class="link ux-fade-in ux-stagger-2">
                    <p class="bold">{{ $t('Link de tu asistente:') }} <a href="/assistantChat">https://inkvaluation.co/assistantChat</a></p>
                    <button class="btnPreview BGYellow ux-button ux-hover-lift">{{ $t('Compartelo') }}</button>
                </div>
            </div>
            <div class="msgDefault ux-card ux-stagger-2">
                <p class="bold ux-fade-in">{{ $t('Tu asistente está listo para compartir') }}</p>
                <p class="ux-fade-in ux-stagger-1">{{ $t('Por defecto preguntará al cliente:') }}</p>
                <ul class="listItemsDefault">
                    <li class="ux-list-item ux-stagger-1">{{ $t('Nombre') }}</li>
                    <li class="ux-list-item ux-stagger-2">{{ $t('Servicio deseado') }}</li>
            ...
            <div class="accountBlock ux-card ux-stagger-3">
                ...
                <div class="divInput ux-form-field ux-stagger-1">
                    <label for="">{{ $t('Nombre del asistente') }}</label>
                    <div class="inputIcon">
                        <input type="text" name="" value="Lola"> 
                    </div>
                </div>
                <div class="divInput ux-form-field ux-stagger-2">
                    <label for="">{{ $t('Mensaje de bienvenida') }}</label>
                    <div class="inputIcon">
                        <textarea  name="">{{ $t('Vamos a comenzar a recopilar la información para realizar tu cotización del servicio.') }}</textarea>
                    </div>
                </div>

                
                
                <button class="btnAccountBlock BGYellow ux-button ux-stagger-3" type="button">{{ $t('Guardar') }}</button>
            </div>
            <div class="accountBlock ux-card ux-stagger-4">
                <h4>{{ $t('Servicios disponibles') }}</h4>
                <p class="parOptForm">{{ $t('Activa los servicios que ofreces para que aparezcan en tu asistente personal.') }}</p>
            ...
            <div class="accountBlock ux-card ux-stagger-5" style="padding-bottom: 0;">
                <h4 style="margin: 0;" class="ux-fade-in">{{ $t('Formularios de servicios') }}</h4>
                <p style="height: auto; margin-bottom: 1rem;" class="ux-fade-in ux-stagger-1">{{ $t('Para cada servicio se debe elegir un formulario que el cliente debe responder al elegir el servicio deseado') }}</p>
                <div class="questionAndType servicesForm titleServicesForm ux-fade-in ux-stagger-2">
                    <div class="serviceColumn">
                        <span class="">{{ $t('Servicio:') }}</span>
                        
                    </div>
                    <div class="serviceColumn">
                        <span class="">{{ $t('Formulario:') }}</span>
